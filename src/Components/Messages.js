...
function Messages({ values, msgId }) {
  ...
  const heartClick = async () => {
    ...
    if (userHeart) {
      ...
      try {
        await db.runTransaction(async (transaction_1) => {
          // This code may get re-run multiple times if there are conflicts.
          const doc_1 = await transaction_1.get(messageDoc);
          if (!doc_1) {
            console.log("doc not found");
            return;
          }
          let newHeartCount_1 = doc_1.data().heartCount + 1;
          let newHeart_1 = doc_1.data().heart ? doc_1.data().heart : {};
          newHeart_1[uid] = true;
          transaction_1.update(messageDoc, {
            heartCount: newHeartCount_1,
            heart: newHeart_1,
          });
        });
        console.log("Liked");
  ...
  const fireClick = async () => {
    ...
    if (userFire) {
      ...
      try {
        await db.runTransaction((transaction_1) => {
          // This code may get re-run multiple times if there are conflicts.
          return transaction_1.get(messageDoc).then((doc_1) => {
            if (!doc_1) {
              console.log("doc not found");
              return;
            }

            let newFireCount_1 = doc_1.data().fireCount + 1;
            let newFire_1 = doc_1.data().fire ? doc_1.data().fire : {};
            newFire_1[uid] = true;

            transaction_1.update(messageDoc, {
              fireCount: newFireCount_1,
              fire: newFire_1,
            });
          });
        });
      ...
      try {
        await db.runTransaction((transaction_1) => {
          ...
          return transaction_1.get(messageDoc).then((doc_1) => {
            ...
            let newLikeCount_1 = doc_1.data().likeCount + 1;
            let newLikes_1 = doc_1.data().likes ? doc_1.data().likes : {};
            newLikes_1[uid] = true;

            transaction_1.update(messageDoc, {
              likeCount: newLikeCount_1,
              likes: newLikes_1,
            });
          });
        });
